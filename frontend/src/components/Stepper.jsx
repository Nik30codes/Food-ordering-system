...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }) {
  ...
  return (
    <motion.div onClick={handleClick} className="step-indicator" style={disableStepIndicators ? { pointerEvents: 'none', opacity: 0.5 } : {}} animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: '#e5e7eb', color: '#666' },
          active: { scale: 1, backgroundColor: '#1a3c34', color: '#1a3c34' },
          complete: { scale: 1, backgroundColor: '#1a3c34', color: '#fff' },
        }}
    ...
    <div className="step-connector">
      <motion.div
        ...
        variants={{
          ...
          complete: { width: '100%', backgroundColor: '#1a3c34' },
        }}
        initial={false}
        ...
        {!isCompleted && (
          <div className="footer-container">
            <div className={`footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
              {currentStep !== 1 && (
                <button onClick={handleBack} className="back-button">{backButtonText}</button>
              )}
              <button onClick={isLastStep ? handleComplete : handleNext} className="next-button">
                {isLastStep ? "Let's Go!" : nextButtonText}
              </button>
            </div>
          </div>
        )}
